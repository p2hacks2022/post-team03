...
class AgaruViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        RoomSelectTitle.frame = CGRect(x: Int(widthTitleCenter), y: 83, width: 250, height: 70)
        RoomSelectTitle.font = UIFont(name: "07NikumaruFont", size: 27)
        BackButton.frame = CGRect(x: 25, y: 83, width: 65, height: 65)
        shareButton.frame = CGRect(x: Int(screenWidth - 25 - 65), y: 83, width: 65, height: 65)
        ...
        screenTapLabel.frame = CGRect(x: Int((screenWidth - 500)/2), y: 720, width: 500, height: 40)
    ...
    @IBAction func tapBackButton(_ sender: Any) {
        ...
        selectTabView.modalTransitionStyle = .crossDissolve
        self.present(selectTabView, animated: true, completion: nil)
    }
